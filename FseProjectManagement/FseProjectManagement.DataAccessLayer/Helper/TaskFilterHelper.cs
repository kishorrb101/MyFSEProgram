...
{
    internal class TaskFilterHelper
    {
        public void CompositeFilter(CompositeFilterDescriptor root, ref IQueryable<TaskDetails> query)
        {
        ...
        public  void Filter(FilterDescriptor filter, ref IQueryable<TaskDetails> query)
        {
            if (filter != null && !string.IsNullOrWhiteSpace(filter.Field) && !string.IsNullOrWhiteSpace(filter.Operator))
            {
                switch (filter.Field.Trim().ToLower())
                {
                    ...
                    case "status":
                        if(filter.FilterOperator == FilterCriteriaDescriptor.NotEqualTo)
                        {
